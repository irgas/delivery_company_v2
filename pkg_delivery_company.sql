CREATE OR REPLACE PACKAGE pkg_delivery_company IS
...
--function that encrypts the password
    ...
    BEGIN
        RETURN dbms_crypto.hash(utl_raw.cast_to_raw(pv_login
        || v_string
        || pv_password),dbms_crypto.hash_sh1);
    END;
...
--function that checks the user's identity
    ...
    BEGIN
        ...
        SELECT
            ...
            password
        INTO
            v_verified,v_blocked,v_password
        FROM
            clients
        WHERE
            upper(login) LIKE upper(p_username);

        IF
            upper(p_password) != upper(v_password)
        THEN
...
--procedures that creating new delivery
    ...
    PROCEDURE p_creating_delivery (
        ...
        CURSOR cur_delivery_details IS SELECT
            c001,
            n001,
            n002,
            n003
    ...
    BEGIN
        ...
        INSERT INTO payers (
            ...
            phone,
            email,
            payer_bank_account_nr
        ) VALUES (
            pv_payer_company_name,
            pv_payer_nip_pesel,
            pv_payer_last_name,
            pv_payer_first_name,
            pv_payer_country,
            pv_payer_city,
            pv_payer_postal_code,
            pv_payer_street,
            pv_payer_building_nr,
            pv_payer_apartment_nr,
            pv_payer_phone,
            pv_payer_email,
            pv_payer_bank_account_nr
        );
        ...
        INSERT INTO delivery (
            ...
            recipient_street,
            recipient_building_nr,
            recipient_apartment_nr,
            recipient_phone,
            recipient_email,
            payer_nip_pesel,
            transport_company_nip_pesel
        ) VALUES (
            v_delivery_nr,
            pn_delivery_cost,
            pv_delivery_content,
            pv_delivery_comments,
            n_distance,
            pv_payment_way,
            'NIEZAP£ACONA',
            pv_sender_company_name,
            pv_sender_nip_pesel,
            pv_sender_last_name,
            pv_sender_first_name,
            pv_sender_country,
            pv_sender_city,
            pv_sender_postal_code,
            pv_sender_street,
            pv_sender_building_nr,
            pv_sender_apartment_nr,
            pv_sender_phone,
            pv_sender_email,
            pv_recipient_company_name,
            pv_recipient_nip_pesel,
            pv_recipient_last_name,
            pv_recipient_first_name,
            pv_recipient_country,
            pv_recipient_city,
            pv_recipient_postal_code,
            pv_recipient_street,
            pv_recipient_building_nr,
            pv_recipient_apartment_nr,
            pv_recipient_phone,
            pv_recipient_email,
            pv_payer_nip_pesel,
            '1118989321'
        ...
        INSERT INTO statues (
            ...
            TO_CHAR(SYSDATE,'YY/MM/DD HH24:MM:SS'),
            v_delivery_nr,
            v_station,
            'Przesy³ka nadana. Czeka na odbiór kuriera.'
        );
...
--function that generate delivery number
    ...
    BEGIN
        SELECT
            'P'
            || lpad(seq_delivery_nr.NEXTVAL,6,'0')
            || '/'
...
--function that returns kilometers for the delivery
    ...
    FUNCTION f_get_distance RETURN NUMBER IS
        n_distance   delivery.distance%TYPE;
    BEGIN
        SELECT
...
--function that returns station
    ...
    FUNCTION f_get_station RETURN VARCHAR2 IS
        v_station   stations.code%TYPE;
    BEGIN
        SELECT
            code
